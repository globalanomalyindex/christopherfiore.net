/**
 * Page 01 ↔ MFNY subpage choreography.
 *
...
/** Stages whose MFNY screen is currently up. */
...
interface Parts {
  ...
  /** the before/after plate — outside the chrome, so it dithers in on its own */
...
function partsFor(stage: HTMLElement): Parts | null {
  const screen = q(stage, '[data-mfny]');
...
function paintScroll(screen: HTMLElement): void {
  ...
  const thumb = q(screen, '[data-mfthumb]');
  if (thumb) {
    if (over <= 1) {
  ...
  const secs = qq(region, '[data-mfsec]');
...
/**
 * Flip the plate between before and after.
 *
 * Both images are already mounted, so this is an opacity swap and a caption
 * write — no decode, no flash, and flipping back and forth costs nothing. The
