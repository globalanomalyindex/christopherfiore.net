/**
 * Page 01 ↔ chipotle subpage choreography.
 *
 * A straight sibling of `runtime/mfny.ts`, and deliberately so: this is the
 * sixth screen built on the same move, and the pattern is the house one. The
 ...
 *
 * Where it differs from mfny: the plate carries four views rather than two, so
 * `setChipotleView` walks a list instead of flipping a pair, and the toggle is
 * a radiogroup of four.
 *
...
/** Stages whose chipotle screen is currently up. */
...
/**
 * True while the chipotle screen is up. `runtime/actions.ts` asks this before
...
function partsFor(stage: HTMLElement): Parts | null {
  const screen = q(stage, '[data-chipotle]');
...
function paintScroll(screen: HTMLElement): void {
  ...
  const thumb = q(screen, '[data-cpthumb]');
  if (thumb) {
    if (over <= 1) {
  ...
  const secs = qq(region, '[data-cpsec]');
...
export function setChipotleView(stage: HTMLElement, n: number): void {
  ...
  const cap = q(screen, '[data-cpcap]');
  if (cap) cap.textContent = view.caption;

  for (const b of qq(screen, '[data-act="chipotle-view"]')) {
