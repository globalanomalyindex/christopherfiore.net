/**
 * Page 01 ↔ lee subpage choreography.
 *
 * A straight sibling of `runtime/mfny.ts`, and deliberately so: this is the
 * seventh screen built on the same move, and the pattern is the house one. The
 ...
 *
 * Where it differs from mfny: the plate carries six views rather than two, so
 * `setLeeView` walks a list instead of flipping a pair, and the toggle is a
 * radiogroup of six. Nothing here counts them, so the count lives in `VIEWS`
 * and nowhere else.
 *
...
import { LEE_LEAD_VIEW as LEAD_VIEW } from '../data/lee.ts';
...
/** Stages whose lee screen is currently up. */
...
/**
 * True while the lee screen is up. `runtime/actions.ts` asks this before
 ...
 * `setLeeView` never touches the column, so the toggle can be walked
 ...
 *
 * A section with no `data-lesec-view` leaves the plate alone: the sections
...
function syncPlateTo(screen: HTMLElement, secs: HTMLElement[], cur: number): void {
  ...
  const raw = secs[cur].getAttribute('data-lesec-view');
...
function paintScroll(screen: HTMLElement): void {
  const region = q(screen, '[data-lescroll]');
  if (!region) return;
  ...
  const thumb = q(screen, '[data-lethumb]');
  if (thumb) {
    if (over <= 1) {
  ...
  const secs = qq(region, '[data-lesec]');
  ...
  const at = q(screen, '[data-lesecat]');
  if (at) {
    const name = secs[cur].getAttribute('data-lesec-name') || '';
    /*
...
function applyView(P: Parts, n: number, animate: boolean): void {
  const { screen } = P;
  const view = LEE_VIEWS[n];
  if (!view) return;
  VIEW.set(screen, n);

  for (const slot of qq(screen, '[data-leslot]')) {
    const on = Number(slot.getAttribute('data-leslot')) === n;
    slot.style.opacity = on ? '1' : '0';
    slot.setAttribute('aria-hidden', on ? 'false' : 'true');
  }
  const cap = q(screen, '[data-lecap]');
  if (cap) cap.textContent = view.caption;

  for (const b of qq(screen, '[data-act="lee-view"]')) {
